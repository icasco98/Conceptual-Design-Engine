...
export function Canvas2D({ width: paneWidth }: { width?: number } = {}) {
  ...
  const runFrame = useCallback(() => {
    ...
    if (g.kind === "move") {
      ...
      let [dx, dy] = toLocalVector(p.x - g.startX, p.y - g.startY, frameOf(box));
      // Shift locks the drag to whichever of the box's own horizontal or
      // vertical axes it is closer to -- the other doesn't move at all.
      if (p.shift) {
        if (Math.abs(dx) >= Math.abs(dy)) dy = 0;
        else dx = 0;
      }
      const [ofx, ofy] = box.points[g.index];
      // Not clamped to the bounding box: a corner can be pushed past it
      // and the shape simply grows there. The box itself (left/top/
      // width/height) is untouched by a vertex edit, so there is no
      // pivot for the growth to shift, and nothing else drifts.
      let fx = ofx + dx / box.width;
      let fy = ofy + dy / box.height;
      // Snapped onto a neighbour's own corner or wall, in this box's own
      // frame so a rotated zone snaps the same as an upright one. Shift
      // means the axis lock above, not this -- the two would fight.
      if (!p.shift) {
        const fr = frameOf(box);
        const outlines = restored.filter((o) => o.id !== box.id).map((o) => pageToLocalPoly(polyOfBox(o), fr));
        const snapped = nearestNeighborPoint([box.left + fx * box.width, box.top + fy * box.height], outlines);
        if (snapped) {
          fx = (snapped[0] - box.left) / box.width;
          fy = (snapped[1] - box.top) / box.height;
        }
      }
      ...
      let oy: number;
      if (p.shift) {
        // Square the wall off instead of holding its own angle: locked
        // to whichever of the box's own horizontal or vertical axes the
        // drag is closer to.
        ...
        // The wall's own direction, in real meters (its two fractions
        // live on different axes that don't share a scale, so the angle
        // has to be worked out after converting out of them) -- fixed
        // for the whole drag, from where the wall started, not
        // recomputed frame to frame as it moves.
        ...
        // A wall with (near) no length has no defined perpendicular;
        // leave its corners exactly where they are rather than
        // guessing one.
        if (wallLen < 1e-6) return;
        // Its normal, and how far along it the pointer has moved: a
        // wall drag only ever pushes the wall in or out, never along
        // its own length or off at an angle, the same as a rectangle's
        // own edge handles are already limited to one direction.
        const nx = -wy / wallLen;
        const ny = wx / wallLen;
        let along = dx * nx + dy * ny;
        // Snapped flush against a nearby wall running the same way, in
        // this box's own frame, same as a vertex.
        const fr = frameOf(box);
        const outlines = restored.filter((o) => o.id !== box.id).map((o) => pageToLocalPoly(polyOfBox(o), fr));
        const a0: Point = [box.left + f0x * box.width + along * nx, box.top + f0y * box.height + along * ny];
        const a1: Point = [box.left + f1x * box.width + along * nx, box.top + f1y * box.height + along * ny];
        along += wallSnapAdjust(a0, a1, [nx, ny], outlines);
        ox = along * nx;
        oy = along * ny;
      }
      // Not clamped to the bounding box, for the same reason a vertex
          ...
          {renderOrder.map((b) => {
            ...
            return (
              ...
              >
                ...
                {/* the bounding box's own corners: not shown on a polygon,
                    whose own corners (below) are what you actually drew --
                    a second, box-shaped set of corners on top of those is
                    confusing, and it is the one whole-shape scale a
                    polygon has, which conflicts with editing it corner by
                    corner and wall by wall. */}
                {solo &&
                  b.shape !== "polygon" &&
